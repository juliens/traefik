...
func (w *grpcWebResponse) prepareHeaders() {
	...
	// Expose all response headers plus gRPC-specific headers for CORS
...
func (w *grpcWebResponse) copyTrailersToPayload() {
	...
	if err := trailers.Write(trailerBuffer); err != nil {
		grpclog.Errorf("failed to write trailers: %v", err)
		return
	}

	// Create gRPC data frame header with MSB=1 to indicate trailer frame
	trailerGrpcDataHeader := make([]byte, 5)
	trailerGrpcDataHeader[0] = 1 << 7 // MSB=1 indicates this is a trailer data frame
	binary.BigEndian.PutUint32(trailerGrpcDataHeader[1:5], uint32(trailerBuffer.Len()))

	if _, err := w.wrapped.Write(trailerGrpcDataHeader); err != nil {
		grpclog.Errorf("failed to write trailer header: %v", err)
	}
	if _, err := w.wrapped.Write(trailerBuffer.Bytes()); err != nil {
		grpclog.Errorf("failed to write trailer data: %v", err)
	}
	flushWriter(w.wrapped)
}
...
func newBase64ResponseWriter(wrapped http.ResponseWriter) *base64ResponseWriter {
...
func (w *base64ResponseWriter) Flush() {
	...
	if err := w.encoder.Close(); err != nil {
		...
		grpclog.Errorf("failed to flush base64 encoder: %v", err)
	}
	w.newEncoder()
	flushWriter(w.wrapped)
}
