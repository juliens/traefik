...
func (t *Toto) Read(b []byte) (int, error) {
	n, _ := t.resp.ReadBodyTest(t.r, t.maxBodySize, b)
	return n, nil
}

func (t *Toto) Close() error {
	return nil
}
...
func (resp *Response) ReadBodyTest(r *bufio.Reader, maxBodySize int, b []byte) (chunkSize int, err error) {
	contentLength := resp.Header.ContentLength()
	if contentLength >= 0 {
		b, err = readBody(r, contentLength, maxBodySize, b)

	} else if contentLength == -1 {
		chunkSize, _, err = readBodyChunkedOne(r, maxBodySize, b)

	} else {
		b, err = readBodyIdentity(r, maxBodySize, b)
...
func readBodyChunkedOne(r *bufio.Reader, maxBodySize int, dst []byte) (int, []byte, error) {
	strCRLFLen := len(strCRLF)

	chunkSize, err := parseChunkSize(r)
	if err != nil {
		return chunkSize, dst, err
	}
	...
	if maxBodySize > 0 && len(dst)+chunkSize > maxBodySize {
		return chunkSize, dst, ErrBodyTooLarge
	}
	...
	if !bytes.Equal(dst[len(dst)-strCRLFLen:], strCRLF) {
		return chunkSize, dst, ErrBrokenChunk{
			error: fmt.Errorf("cannot find crlf at the end of chunk"),
		}
	}
	dst = dst[:len(dst)-strCRLFLen]
	return chunkSize, dst, nil
}
