...
func (c *Client) GetHostClient(req *Request) (*HostClient, error) {
	uri := req.URI()
	if uri == nil {
...
func (c *HostClient) doNonNilReqResp(req *Request, resp *Response) (bool, error) {
	...
	if (resp.Header.ContentLength() > 0 && resp.mustSkipBody()) || resetConnection || req.ConnectionClose() || resp.ConnectionClose() || isConnRST {
		c.closeConn(cc)
	} else {
		c.releaseConn(cc)
	}
...
func (c *HostClient) ReleaseConnObject(conn *ConnObject) {
	c.releaseWriter(conn.Bw)
	c.releaseReader(conn.Br)
	c.releaseConn(conn.c)
}

func (c *HostClient) AcquireconnObject() (*ConnObject, error) {
	conn, err := c.acquireConn(0, false)
	if err != nil {
		return nil, err
	}
	return &ConnObject{c: conn, Bw: c.acquireWriter(conn.c), Br: c.acquireReader(conn.c)}, nil
}
