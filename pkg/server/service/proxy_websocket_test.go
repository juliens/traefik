...
func TestWebSocketTCPClose(t *testing.T) {
	f, err := buildProxy(nil, http.DefaultTransport, nil)
...
func TestWebSocketTransferTLSConfig(t *testing.T) {
	...
	forwarderWithoutTLSConfig, err := buildProxy(nil, http.DefaultTransport, nil)
	require.NoError(t, err)
	...
	forwarderWithTLSConfig, err := buildProxy(nil, transport, nil)
	require.NoError(t, err)
	...
	forwarderWithTLSConfigFromDefaultTransport, err := buildProxy(nil, http.DefaultTransport, nil)
	require.NoError(t, err)
